...
public class ProductService : IProductService
{
    ...
    {
        HttpClient client = _clientFactory.CreateClient("ProductApi");
        ...
        using (var response = await client.GetAsync(apiEndpoint))
        {
            if (response.IsSuccessStatusCode)
            {
                ...
                productsVM = await JsonSerializer.DeserializeAsync<IEnumerable<ProductViewModel>>(apiResponse, _jsonSerializer);
            }
            else
                return null;
        }
    ...
    public async Task<ProductViewModel> FindProductById(int id)
    {
        ...
        using (var response = await client.GetAsync(apiEndpoint + id))
        {
            ...
            {
                var apiResponse = await response.Content.ReadAsStreamAsync();
                productVM = await JsonSerializer.DeserializeAsync<ProductViewModel>(apiResponse, _jsonSerializer);
            }
            else
                return null;
        }
    ...
    public async Task<ProductViewModel> UpdateProduct(ProductViewModel productVM)
    {
        ...
        using (var response = await client.PutAsJsonAsync(apiEndpoint, productVM))
        {
            ...
            {
                var apiResponse = await response.Content.ReadAsStreamAsync();
                productUpdated = await JsonSerializer.DeserializeAsync<ProductViewModel>(apiResponse, _jsonSerializer);
            }
            else
                return null;
        }
